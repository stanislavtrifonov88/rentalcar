...
export const transformToContractDTO = async (
  contract: Contract,
): Promise<IndividualContractDTO> => {
  const contractInfo = (({ id, startDate, contractEndDate }): any => ({
    id,
    startDate,
    contractEndDate,
  }))(contract);
  const carInfo = (({ brand, model }): any => ({ brand, model }))(contract.car);
  const customerInfo = (({ phone, firstName, lastName, birthdate }): any => ({
    phone,
    firstName,
    lastName,
    birthdate,
  ...
  const individualContractFormated: IndividualContractDTO = {
    ...contractInfo,
    ...carInfo,
    ...customerInfo,
    age,
